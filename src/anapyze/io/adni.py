...
def filter_ADNI_mri_csv(mri_csv, output_csv):
    """
        ...
        for age in ages:
            ...
            if age_df.empty:
                ...
                if len(age_df) == 1:
                    ...
                    accepted_descriptions = {'Accelerated SAG IR-SPGR', 'MPRAGE SENSE2', 'MPRAGE GRAPPA2',
                                             'MPRAGE_GRAPPA2', 'Accelerated Sag IR-FSPGR', 'IR-SPGR w/acceleration'
                                             'Accelerated Sagittal MPRAGE',
                                             'Accelerated SAG IR-FSPGR', 'Accelerated Sag IR-SPGR',
                                             'Accelerated Sagittal MPRAGE', 'MPRAGE SENSE2 SENSE', 'MPRAGE SENSE',
                                             'Sagittal 3D Accelerated MPRAGE', 'Sagittal 3D Accelerated 0 angle MPRAGE',
                                             'ACCELERATED SAG IR-SPGR', 'Accelerated Sagittal IR-FSPGR',
                                             'IR-SPGR w/acceleration', 'Accelerated Sagittal MPRAGE Phase A-P',
                                             'Sag Accel IR-FSPGR', 'Accelerated_Sag_IR-FSPGR',
                                             'MPRAGE_GRAPPA2          straight no angle', 'MPRAGE SENS',
                                             'MPR; GradWarp; B1 Correction; N3; Scaled <- MP-RAGE', 'MPRAGE REPEAT',
                                             'SAG IR-FSPGR-Repeat', 'IR-FSPGR-Repeat', 'IR-FSPGR REPEAT',
                                             'MPRAGE_ASO_repeat', 'MPRAGE_ Sag  - NO ANGLE=', 'MPRAGE Repeat',
                                             'MP-RAGE REPEAT', 'MP RAGE SAGITTAL REPEAT', 'MP-RAGE-REPEAT',
                                             'MP-RAGE-Repeat', 'MP-RAGE repeat', 'MP-RAGE', 'Sag IR-FSPGR Repeat',
                                             'MP RAGE REPEAT', 'ADNI       MPRAGE', 'MPRAGE', 'ASO-MPRAGE',
                                             'MPRAGE AUTOSHIM ON', 'Sag IR-SPGR', 'MPRAGE SAGITTAL', 'MPRAGE ASO',
                                             'ADNI SH    MPRAGE ASO', 'MPRAGE SAG', 'ADNI-R11   MPRAGE-REPEA',
                                             'ADNI-R11   MPRAGE', 'ADNI-R11-ASASO-MPRAGE', 'ADNI-R11-ASASO-MPRAGE(2',
                                             ...
                                             'SAG MPRAGE GRAPPA2 NO ANGLE', 'SAG MPRAGE NO ANGLE', 'SAG MP-RAGE REPEAT',
                                             'SAG MP-RAGE'
    ...
    filtered_mri_df.to_csv(output_csv, index = False)

def is_ADNI_subject_amyloid_PET_positive(amyloid_csv, subject_id = False, rid = False, date = 'baseline'):
    """
...
def get_csf_biomarkers_ADNI(csf_csv, asyn_csf_csv, subject_id = False, rid = False, date = 'baseline'):
    """
    ...
    asyn_csf_df = pd.read_csv(asyn_csf_csv)
    ...
    df_subj = csf_df.loc[csf_df['RID'] == rid]
    ...
    df_subj = asyn_csf_df.loc[asyn_csf_df['RID'] == rid]
...
def get_genetics_data_ADNI(apoe_csv, ageing_genes_csv, subject_id = False, rid = False):
    """
    ...
    apoe_df = pd.read_csv(apoe_csv)
    adni_aging_genes_df = pd.read_csv(ageing_genes_csv)
    ...
    filtered_apoe = apoe_df[apoe_df.RID == rid]
    ...
    genes = adni_aging_genes_df.loc[adni_aging_genes_df['RID'] == rid]
...
def get_cognition_data_ADNI(mmse_csv, composites_csv, subject_id = False, rid = False, date = 'baseline'):

    # TODO: This function need to be updated to acommodate latest ADNI changes

    mmse_df = pd.read_csv(mmse_csv)
    composites_df = pd.read_csv(composites_csv)
    ...
    filtered_mmse = mmse_df[mmse_df.RID == rid]
    ...
    filtered_composite = composites_df[composites_df.RID == rid]
...
def get_neuropsychological_battery_ADNI(neurobat_csv, subject_id = False, rid = False, date = 'baseline'):

    # REF: https://adni.bitbucket.io/reference/neurobat.html

    neurobat_df = pd.read_csv(neurobat_csv)
    ...
    filtered_npb = neurobat_df[neurobat_df.RID == rid]
...
def get_wmh_ADNI(wmh_csv, subject_id = False, rid = False, date = 'baseline'):

    wmh_df = pd.read_csv(wmh_csv)
    ...
    filtered_wmh = wmh_df[wmh_df.RID == rid]
